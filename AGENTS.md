...
- **Export to Excel / Google Sheets**: User list can be exported as `.xlsx` or to a new Google Spreadsheet. Full dataset (all pages) is fetched client-side then enriched via `/api/users/taskinfo` (respects existing 5-concurrent/100ms rate limit). Excel export runs client-side via `xlsx` library (`src/lib/export/excelExport.ts`). Sheets export goes through `POST /api/users/export-sheets` (server-side, creates new spreadsheet via Sheets REST API). Google Sheets uses a **separate** OAuth flow (`/api/auth/sheets/*`) from Gmail — scoped to `drive.file` only. Refresh token stored at Redis key `sheetsaccount:{username}` (single-account model, see `src/lib/sheetsAccount.ts`). Additional env var required: `GOOGLE_SHEETS_OAUTH_REDIRECT_URI` (e.g. `http://localhost:3000/api/auth/sheets/callback`). The "Google Sheets for Export" connect/disconnect UI lives in `EmailDialog.tsx`.
