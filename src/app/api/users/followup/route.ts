...
const COUNTRY_CONFIG: Record<string, { code: string; minLength: number }> = {
    mx: { code: '52', minLength: 10 }, // Mexico:   always 10 digits
    pe: { code: '51', minLength: 9  }, // Peru:     9 (mobile); use 8 to avoid over-stripping
...
const DEFAULT_MIN_LOCAL_LENGTH = 10;

function getCountryConfig(baseUrl: string): { countryCode: string; minLocalLength: number } {
    try {
